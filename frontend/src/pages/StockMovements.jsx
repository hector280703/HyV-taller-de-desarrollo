...
const TIPO_CONFIG = {
  entrada: { label: 'Entrada', icon: '📥', className: 'tipo-entrada' },
  salida: { label: 'Salida', icon: '📤', className: 'tipo-salida' },
  ajuste: { label: 'Ajuste', icon: '🔧', className: 'tipo-ajuste' },
  devolucion: { label: 'Devolución', icon: '↩️', className: 'tipo-devolucion' },
};
  ...
  return (
    <div className="stock-movements-container">
        ...
        {Object.entries(TIPO_CONFIG).map(([tipo, config]) => (
          <div key={tipo} className={`summary-card ${config.className}`}>
            ...
            <div>
              ...
              <p className="summary-label">{config.label}s</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="movements-empty"><span>📭</span><p>No hay movimientos registrados</p></div>
      ) : (
            ...
            <form onSubmit={handleCreateMovement} className="movement-form">
              ...
              {formData.tipo === 'ajuste' && (
                <div className="ajuste-info">
                  ⚠️ Para <strong>Ajuste</strong>, la cantidad ingresada será el <strong>nuevo stock total</strong> del producto.
                </div>
              )}
